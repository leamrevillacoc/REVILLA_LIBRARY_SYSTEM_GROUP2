...
def borrow_book(username):
    ...
    if book_id in books:
        if books[book_id]["available"]:
            confirm = input(f"Are you sure you want to borrow '{books[book_id]['title']}'? (yes/no): ").strip().lower()
            if confirm != "yes":
                print("❌ Borrowing canceled.\n")
                return
...
def add_book():
    new_id = input("Enter new Book ID: ").strip()
    title = input("Enter Book Title: ").strip()
    shelf = input("Enter Shelf Location (e.g., Shelf A): ").strip()
